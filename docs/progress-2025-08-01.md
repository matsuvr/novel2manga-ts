...
本日は、OpenTelemetryの依存関係問題の解決と、プロジェクトの軽量化・最適化に集中しました。Mastra の導入方針を撤回し、AI呼び出しは OpenAI SDK / Google GenAI SDK を直接使用する構成に変更しました。これにより不要な依存関係を大幅に削減し、システムの安定性を向上させました。
...
- **ライブラリ使用の明確化**: OpenAI/GenAI を直接利用
