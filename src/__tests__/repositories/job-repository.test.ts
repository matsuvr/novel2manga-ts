...
import { JobRepository, type JobDbPort } from '@/repositories/job-repository'
import type { Job } from '@/db'

function createMockPort() {
  const calls: { createWithId: Array<[string, string, string | undefined]>; createPayload: any[] } = {
    createWithId: [],
    createPayload: [],
  }
  const mock: JobDbPort = {
    ...
    async createJob(idOrPayload: string | { novelId: string; title?: string; totalChunks?: number; status?: string }, novelId?: string, jobName?: string) {
