...
import {
  ...
  createTestDatabase,
  TestDataFactory,
} from "./__helpers/test-database";
import {
  TestStorageDataFactory,
  TestStorageFactory,
} from "./__helpers/test-storage";
  ...
  const factory = () => ({
    getJobRepository: () => ({
      ...
      getJobWithProgress: (id: string) =>
        __testDbForFactory!.service.getJobWithProgress(id),
    }),
    getNovelRepository: () => ({
      get: (id: string) => __testDbForFactory!.service.getNovel(id),
      ensure: (id: string, payload: any) =>
        __testDbForFactory!.service.ensureNovel(id, payload),
    }),
    getChunkRepository: () => ({
      create: (payload: any) =>
        __testDbForFactory!.service.createChunk(payload),
      createBatch: (payloads: any[]) =>
        __testDbForFactory!.service.createChunksBatch(payloads),
      getByJobId: (jobId: string) =>
        __testDbForFactory!.service.getChunksByJobId(jobId) as any,
      db: {
...
describe("Service Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    // エージェントモックのセットアップ
    setupAgentMocks();
    ({ AnalyzePipeline } = await import(
      "@/services/application/analyze-pipeline"
    ));
  });
  ...
  describe("Database and Storage Integration", () => {
    it("データベースとストレージ間のデータ一貫性を保つ", async () => {
      ...
      for (const chunk of chunks) {
        const chunkStorage = await testStorageFactory.getChunkStorage();
        const storedChunk = await chunkStorage.get(
          `${result.jobId}/chunks/${chunk.chunkIndex}.txt`
        );
