// integration: NovelDatabaseService tests (restored from unit)
...
describe('NovelDatabaseService', () => {
  ...
  it('ensureNovel does not throw and accepts explicit id', async () => {
    const { db, adapter } = makeFakeDb()
    const service = new NovelDatabaseService(db as any, adapter as any)
    await service.ensureNovel('n1', {
      userId: 'u1',
      title: 't',
      author: 'a',
      originalTextPath: '/x',
