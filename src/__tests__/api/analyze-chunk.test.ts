  ...
  beforeEach(async () => {
    ...
    const mockChunkStorage = {
      get: vi.fn().mockImplementation((path: string) => {
        if (path === `chunks/${testJobId}/chunk_0.txt`) {
          return { text: 'これはテスト用のチャンクテキストです。分析対象のサンプルテキストです。' }
        }
        if (path === `chunks/${testJobId}/chunk_1.txt`) {
          return { text: '2番目のチャンクテキストです。継続する物語の内容です。' }
        }
        if (path === `chunks/${testJobId}/chunk_999.txt`) {
      ...
      exists: vi.fn().mockImplementation((path: string) => {
        return path !== `chunks/${testJobId}/chunk_999.txt` && !path.startsWith('nonexistent-job')
      }),
    }

    const mockAnalysisStorage = {
      get: vi.fn().mockImplementation((path: string) => {
        if (path === `analyses/${testJobId}/chunk_1.json`) {
      ...
      delete: vi.fn(),
      exists: vi.fn().mockImplementation((path: string) => {
        return path === `analyses/${testJobId}/chunk_1.json`
      }),
    }
