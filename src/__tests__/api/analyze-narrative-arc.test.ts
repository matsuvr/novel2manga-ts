  ...
  describe("POST /api/analyze/narrative-arc", () => {
    it("有効なリクエストでナラティブアーク分析を実行する", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/analyze/narrative-arc",
        {
          method: "POST",
    ...
    it("準備関数が失敗した場合は400エラーを返す", async () => {
      ...
      const { prepareNarrativeAnalysisInput } = await import(
        "@/utils/episode-utils"
      );
      ...
      expect(data.details).toBeDefined();
      expect(data.details.reason).toBe(
        "Not enough chunks available or invalid chunk range"
      );
      expect(data.details.code).toBe("INVALID_INPUT");
    });

    it("分析関数がエラーを投げた場合は500エラーを返す", async () => {
      // analyzeNarrativeArcがエラーを投げるようにモック
      const { analyzeNarrativeArc } = await import(
        "@/agents/narrative-arc-analyzer"
      );
      ...
      expect(data.details).toBeDefined();
      expect(data.details.original).toBe("分析処理でエラーが発生しました");
    });
