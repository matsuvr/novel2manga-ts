...
{{nextChunkText}}`,
    ...
    layoutGeneration: {
      ...
      // 【ここを設定】レイアウト生成の指示
      instructions: `あなたは日本のマンガのコマ割り（パネルレイアウト）を生成する専門家です。
      
      重要なルール：
      1. 田の字のような均等なグリッド分割は絶対に避ける
      2. コマのサイズに変化をつけて、視覚的なリズムを作る
      3. 重要なシーンは大きなコマで表現する
      4. 読みやすさを考慮し、右上から左下への視線の流れを意識する
      5. 1ページあたりのコマ数は3-6コマを基本とする
      6. ハイライトシーンは通常の2-3倍のサイズで表現する
      
      入力されたエピソードデータから、適切なコマ割りを決定し、
      各コマの内容とセリフを配置してください。`,
