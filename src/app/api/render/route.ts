...
export async function POST(request: NextRequest) {
  try {
    ...
    const targetPage = (mangaLayout as any).pages.find(
      (p: any) => p.page_number === body.pageNumber,
    )
    ...
    if (typeof (dbService as any).getJob === 'function') {
      const job = await (dbService as any).getJob(body.jobId)
      if (!job) return validationError('指定されたジョブが見つかりません')
    }
    if (typeof (dbService as any).getEpisodesByJobId === 'function') {
      const episodes = await (dbService as any).getEpisodesByJobId(body.jobId)
      const targetEpisode = episodes.find((e: any) => e.episodeNumber === body.episodeNumber)
      if (!targetEpisode)
    ...
    // ステータス更新（存在する場合）
    if (typeof (dbService as any).updateRenderStatus === 'function') {
      const fn = (dbService as any).updateRenderStatus
      try {
        await fn(body.jobId, body.episodeNumber, body.pageNumber, {
          isRendered: true,
          imagePath: renderKey,
