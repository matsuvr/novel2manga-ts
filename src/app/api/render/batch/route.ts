...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const targetEpisode = episodes.find(
      (e) => e.episodeNumber === validatedBody.episodeNumber
    );
    if (!targetEpisode) {
      return validationError(
        `エピソード ${validatedBody.episodeNumber} が見つかりません`
      );
    }

    // レンダリング対象ページの決定
    const targetPages =
      validatedBody.pages || mangaLayout.pages.map((p) => p.page_number);
    ...
    const renderPage = async (pageNumber: number) => {
      try {
        ...
        // サムネイル生成
        const thumbnailBlob = await ThumbnailGenerator.generateThumbnail(
          imageBlob,
          {
            width: 200,
            height: 280,
