  ...
  try {
    ...
    if (globalThis.DB) {
      ...
      await db
        ...
        .bind(uuid, fileName, length, totalChunks, chunkSize, overlapSize)
        .run()
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (globalThis.DB) {
      ...
      if (id) {
        const novel = await db.prepare(`SELECT * FROM novels WHERE id = ?`).bind(id).first()
        ...
        const jobs = await db
          .prepare(`SELECT * FROM jobs WHERE novel_id = ? ORDER BY created_at DESC`)
          .bind(id)
          .all()

        return NextResponse.json({ novel, jobs: jobs.results })
      } else {
        const result = await db.prepare(`SELECT * FROM novels ORDER BY created_at DESC`).all()
