...
import { createErrorResponse, NotFoundError } from "@/utils/api-error";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!episode) {
      throw new NotFoundError("Episode", String(episodeNumber));
    }
    ...
    for (let i = ensuredEpisode.startChunk; i <= ensuredEpisode.endChunk; i++) {
      ...
      try {
        ...
        const isPartial =
          i === ensuredEpisode.startChunk || i === ensuredEpisode.endChunk;
        const startOffset =
          i === ensuredEpisode.startChunk ? ensuredEpisode.startCharIndex : 0;
        const endOffset =
          i === ensuredEpisode.endChunk
            ? ensuredEpisode.endCharIndex
    ...
    if (chunkDataArray.length === 0) {
      throw new NotFoundError("Chunk analysis data");
    }
    ...
    const outputPath = path.join(
      outputDir,
      `episode_${episodeNumber}_layout.yaml`
    );
