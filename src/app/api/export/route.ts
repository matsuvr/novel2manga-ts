...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    // エピソードの取得
    const allEpisodes = await dbService.getEpisodesByJobId(body.jobId);
    ...
    switch (body.format) {
      case "pdf":
        ({ exportFilePath, fileSize, pageCount } = await exportToPDF(
          body.jobId,
          targetEpisodes
        ));
        break;
      case "images_zip":
        ({ exportFilePath, fileSize, pageCount } = await exportToZIP(
          body.jobId,
          targetEpisodes
        ));
        break;
    ...
    console.log(
      `エクスポート完了: ${exportFilePath} (${fileSize} bytes, ${pageCount} pages)`
    );
...
async function exportToPDF(
  ...
  for (const episode of episodes.sort(
    ...
    try {
      ...
      console.warn(
        `レイアウト解析エラー Episode ${episode.episodeNumber}:`,
        error
      );
      continue;
    }
    ...
    if (mangaLayout.pages) {
      for (const page of mangaLayout.pages.sort(
...
async function exportToZIP(
  ...
  for (const episode of episodes.sort(
    ...
    try {
        ...
        if (mangaLayout.pages) {
          for (const page of mangaLayout.pages.sort(
            ...
            try {
              ...
              if (imageData?.text) {
                const imageBuffer = Buffer.from(imageData.text, "base64");
                const fileName = `page_${page.page_number
                  .toString()
                  .padStart(3, "0")}.png`;
      ...
      console.warn(
        `レイアウト処理エラー Episode ${episode.episodeNumber}:`,
        error
      );
    }
  }
