...
import { VerticalTextRequestSchema } from '@/services/vertical-text/schemas'
...
export async function POST(req: Request): Promise<Response> {
  const program = Effect.gen(function* (_) {
    ...
    const body = yield* _(Schema.decodeUnknown(VerticalTextRequestSchema)(json)).pipe(
      Effect.mapError(
        (errors) =>
          new ValidationError({
    ...
    const result = yield* _(service.render(body))
