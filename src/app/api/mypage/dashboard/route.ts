    ...
    Effect.gen(function* () {
      ...
      const novels = yield* Effect.tryPromise({
        try: () => db.novels().getAllNovels(userId),
          ...
          new ApiError('Failed to fetch novels', 500, 'DATABASE_ERROR', { error }),
      })

      const jobs = yield* Effect.tryPromise({
        try: () => db.jobs().getJobsByUser(userId),
        catch: (error) =>
          new ApiError('Failed to fetch jobs', 500, 'DATABASE_ERROR', { error }),
      })

      const outputs = yield* Effect.tryPromise({
        try: () => db
          .outputs()
          .getOutputsByUserId(userId, mypageConfig.recentOutputsLimit),
        catch: (error) =>
          new ApiError('Failed to fetch outputs', 500, 'DATABASE_ERROR', { error }),
      })
