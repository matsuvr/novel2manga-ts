...
function ProcessingProgress({ jobId, onComplete }: ProcessingProgressProps) {
  ...
  const updateStepsFromJobData = useCallback(
    (data: JobData) => {
      ...
      addLog(
        "info",
        `Job状態更新: ${data.job.status} - ${data.job.currentStep}`
      );
      ...
      setSteps((prevSteps) => {
        ...
        if (data.job.status === "completed") {
          ...
          if (data.job.splitCompleted) {
            ...
            if (
              data.job.totalChunks &&
              data.job.processedChunks !== undefined
            ) {
          ...
          if (data.job.layoutCompleted) {
            ...
            if (
              data.job.totalEpisodes &&
              data.job.processedEpisodes !== undefined
            ) {
        ...
        setOverallProgress(
          Math.round((completedCount / INITIAL_STEPS.length) * 100)
        );
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `API呼び出しに失敗: ${response.status} ${response.statusText}`
          );
        }
