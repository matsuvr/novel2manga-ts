...
/**
 * Repository interface for loading chunk analyses.
 */
export interface IChunkRepository {
  getAnalysis(jobId: string, chunkIndex: number): Promise<ChunkAnalysisResult | null>
}

/**
 * Bundle analyzer interface for aggregating chunk analyses.
 */
export interface IBundleAnalyzer {
  analyze(
    chunks: Array<{ text: string; analysis: ChunkAnalysisResult }>,
  ): Promise<BundleAnalysisResult>
}

/**
 * Default chunk repository using StorageFactory.
 */
class StorageChunkRepository implements IChunkRepository {
  async getAnalysis(jobId: string, chunkIndex: number): Promise<ChunkAnalysisResult | null> {
...
/**
 * Default bundle analyzer delegating to existing agent implementation.
 */
class DefaultBundleAnalyzer implements IBundleAnalyzer {
  async analyze(
    chunks: Array<{ text: string; analysis: ChunkAnalysisResult }>,
  ): Promise<BundleAnalysisResult> {
    const { analyzeChunkBundle } = await import('@/agents/chunk-bundle-analyzer')
    return analyzeChunkBundle(chunks)
  }
}

/**
 * Narrative arc analyzer orchestrates bundle analysis and LLM processing.
 */
export class NarrativeArcAnalyzer {
  constructor(
    private readonly chunkRepository: IChunkRepository,
    private readonly bundleAnalyzer: IBundleAnalyzer,
  ) {}
  ...
  async analyze(input: {
    ...
    const chunksWithAnalyses = [] as Array<{
      text: string
      analysis: ChunkAnalysisResult
    }>
    for (const chunk of input.chunks) {
      console.log(`Loading analysis for chunk ${chunk.chunkIndex}...`)
      const analysisResult = await this.chunkRepository.getAnalysis(input.jobId, chunk.chunkIndex)
    ...
    try {
      bundleAnalysis = await this.bundleAnalyzer.analyze(chunksWithAnalyses)
    } catch (error) {
    ...
    const responseSchema = z.object({
      boundaries: z.array(
        z.object({
          startPosition: z
            .number()
            .describe('エピソード開始位置（全文テキストの先頭からの文字数）'),
...
/**
 * Factory to create analyzer with default implementations.
 */
export function createNarrativeArcAnalyzer(
  chunkRepository: IChunkRepository = new StorageChunkRepository(),
  bundleAnalyzer: IBundleAnalyzer = new DefaultBundleAnalyzer(),
): NarrativeArcAnalyzer {
  return new NarrativeArcAnalyzer(chunkRepository, bundleAnalyzer)
}

/**
 * Convenience function retaining previous API.
 */
export async function analyzeNarrativeArc(input: {
  ...
  const analyzer = createNarrativeArcAnalyzer()
