...
} from "@/agents/chunk-bundle-analyzer";
...
export async function analyzeNarrativeArc(
  ...
  const targetPages = Math.round(
    input.targetCharsPerEpisode / episodeConfig.charsPerPage
  );
  const minPages = Math.round(
    input.minCharsPerEpisode / episodeConfig.charsPerPage
  );
  const maxPages = Math.round(
    input.maxCharsPerEpisode / episodeConfig.charsPerPage
  );
  ...
  const analyzedChunks = await chunkRepository.getAnalyzedChunks(
    input.jobId,
    chunkIndices
  );
  const analysisMap = new Map(
    analyzedChunks.map((c) => [c.chunkIndex, c.analysis])
  );
    ...
    return {
      ...
      highlights: partial.highlights.map((h) => ({
        ...
        content:
          ...
          text.substring(
            h.startIndex,
            Math.min(h.endIndex, h.startIndex + 120)
          ),
        intensity: 5,
        relevance: 5,
      })),
  ...
  for (const chunk of input.chunks) {
    const analysisCandidate =
      chunk.analysis || (analysisMap.get(chunk.chunkIndex) as unknown);
    ...
    const normalized = toFullAnalysis(
      chunk.chunkIndex,
      chunk.text,
      analysisCandidate
    );
  ...
  console.log(
    `Successfully prepared ${chunksWithAnalyses.length} chunk analyses`
  );
  ...
  try {
    ...
    throw new Error(
      "Failed to perform bundle analysis before narrative arc analysis"
    );
  }

  console.log(
    "Bundle analysis completed, proceeding to narrative arc analysis..."
  );
...
`;
  ...
  const responseSchema = z.object({
    boundaries: z.array(
      z.object({
        ...
        endPosition: z
          .number()
          .describe("エピソード終了位置（全文テキストの先頭からの文字数）"),
        episodeNumber: z
          .number()
          .describe(
            `エピソード番号（${input.startingEpisodeNumber || 1}から開始）`
          ),
  ...
  try {
    ...
    if (!result.object) {
      const errorMsg =
        "Failed to generate narrative analysis - LLM returned no object";
...
function convertPositionsToBoundaries(
  ...
  // 位置からチャンク番号を找す関数
  const findChunkAndOffset = (
    position: number
  ): { chunkIndex: number; charIndex: number } => {
