  ...
  switch (provider) {
    ...
    case "openrouter":
      if (!providerCache.openrouter) {
        const providerConfig: ProviderConfig = config as ProviderConfig;
        const baseURL =
        ...
        const shouldUseCerebras =
          providerConfig.preferCerebras && cerebrasModel;

        const openrouterConfig: OpenAIProviderSettings = { apiKey, baseURL };
          ...
          openrouterConfig.fetch = async (
            ...
            if (init && typeof init.body === "string" && init.body.length > 0) {
              try {
                ...
                parsed.provider = { order: ["cerebras"] };
...
export async function getProviderWithFallback(preferredProvider?: LLMProvider) {
  ...
  throw new Error(
    `All LLM providers failed to initialize. Last error: ${lastError?.message}`
  );
}
...
export async function getTextAnalysisLLM() {
  ...
  // モデルオーバーライドがあれば適用
  let model =
    config.modelOverrides?.[llmInstance.providerName] || llmInstance.model;

  // OpenRouterでCerebras対応が有効な場合、Cerebrasモデルを使用
  if (
    llmInstance.providerName === "openrouter" &&
    (llmInstance.config as ProviderConfig).preferCerebras
  ) {
