...
type MinimalD1Database = {
  ...
  >
  exec: (query: string) => Promise<{ count?: number; duration?: number; [k: string]: unknown }>
}
...
export class D1Adapter implements DatabaseAdapter {
  ...
  async run(query: string, params?: unknown[]): Promise<QueryResult> {
    ...
    return {
      ...
      lastInsertRowid: typeof result.last_row_id === 'number' ? result.last_row_id : undefined,
      success: result.success === true,
      meta: Object.fromEntries(
        Object.entries(result).filter(([k]) => !['changes', 'last_row_id', 'success'].includes(k)),
      ),
    }
  }
  ...
  async batch(statements: PreparedStatement[]): Promise<QueryResult[]> {
    ...
    return results.map((r) => ({
      ...
      lastInsertRowid: typeof r.last_row_id === 'number' ? r.last_row_id : undefined,
      success: r.success === true,
      meta: Object.fromEntries(
        Object.entries(r).filter(([k]) => !['changes', 'last_row_id', 'success'].includes(k)),
      ),
    }))
  }
