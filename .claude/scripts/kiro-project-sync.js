      ...
      for (const line of lines) {
        if (line.startsWith('## ')) {
          ...
          if (title.toLowerCase().includes('critical') || title.toLowerCase().includes('重要')) {
            priority = 'High';
          } else if (title.toLowerCase().includes('optional') || title.toLowerCase().includes('任意')) {
            priority = 'Low';
          }
  ...
  async createProjectItem(task, fieldIds) {
    try {
      ...
      const updates = [
        ...
        { fieldId: fieldIds.Phase, optionId: fieldIds.Phase_options[task.phase] || fieldIds.Phase_options.Implementation },
