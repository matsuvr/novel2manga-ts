...
- [x] **複数吹き出しの水平配置**: 同一コマに複数吹き出しがある場合は横並びにし、吹き出しを最優先で配置
  ...
  - [x] **描画処理の重複削除**: 二重に行われていた吹き出し描画と話者ラベルのクランプ処理を整理
  - [x] **吹き出しスケール精度向上**: 幾何学的スケールとパディングを考慮して幅・高さを計算し、単一/複数吹き出しとも最大領域を厳密に遵守
  - [x] **吹き出し重複描画削除**: 二重に行われていた吹き出し描画と話者ラベルのクランプ処理を整理
  - [x] **状況説明テキスト拡張**: フォントサイズと描画領域を2倍に拡大し、PanelLayoutCoordinatorでSFX・吹き出しとの重なりを防止
...
### Refactor: render status transaction deduplication (2025-09-11)
