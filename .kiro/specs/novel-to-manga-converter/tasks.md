...
- [ ] **16. Job Status API 基本動作修正**
  - [ ] `DatabaseService.getJobWithProgress()` の例外ハンドリング修正
  - [ ] nullチェックとエラー時のフォールバック実装
  - [ ] `/api/jobs/[jobId]/status` エンドポイントの基本動作確保
  - [ ] データベース接続テストの実装
...
- [ ] **18. 分析パイプライン最小実装**
  - [ ] 固定値による分析結果生成（Mastraを一旦迂回）
  - [ ] チャンク処理の基本フロー動作確認
  - [ ] エラーハンドリングとJob状態更新の実装
  - [ ] 簡単なテキスト処理での疎通テスト
