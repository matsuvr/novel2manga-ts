...
// 注意: このシナリオは旧設計に基づく可能性があり、最新のUI/APIフローと乖離している場合があります。
// 現行仕様に整合するまでスキップしておきます（E2E安定化のため）。
test.describe.skip(
  'LEGACY: E2E: analyze → chunk-scripts → merge → page-break → bundle (JSON) → status',
  () => {
    test('process sample novel and complete full pipeline (JSON format)', async ({
      request,
      page,
    }) => {
      ...
      // Prewarm Next.js dev server routes to avoid first-hit compilation timeout
      await page.goto('/')
