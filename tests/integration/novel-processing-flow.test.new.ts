...
describe('小説処理フロー統合テスト', () => {
  ...
  it('小説の分割（チャンク化）', async () => {
    ...
    console.log(`✓ チャンク分割成功: ${result.chunks.length}個のチャンクを生成`)
  }, 300000) // 5分のタイムアウト
  ...
  it('テキスト分析の実行', async () => {
    ...
    if (!analysis || !analysis.jobId) {
      throw new Error('分析ジョブIDが見つかりません')
    }

    // チャンクが既に生成されているので、分析結果を確認
    expect(chunks).toBeDefined()
    expect(chunks.length).toBeGreaterThan(10)
    expect(analysis.jobId).toBeDefined()

    console.log(`✓ テキスト分析完了: ${chunks.length}チャンクが既に分析済み`)
  }, 60000) // 1分のタイムアウト
